...
const cmd = [].concat([
  ...
  'echo "### checkout|create major-branch ' + newMajorBranchName + ', set upstream to github" &&',
  'git checkout -B ' + newMajorBranchName + ' &&',
  '# when major branch does not exist on remote, then push it to github',
  'git ls-remote --exit-code . github/' + newMajorBranchName + ' || git push github ' + newMajorBranchName + ' -u  &&',
  '# pull the current upstream state',
  'git fetch --all &&',
  '# update major branch with changes from this release',
  'git merge v' + newVersion + ' &&',
  'git push github ' + newMajorBranchName + ' -u &&',
